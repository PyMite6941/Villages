...
export default function PostCard({ post, showComments = false }: Props) {
  ...
  return (
    <div className={`card ${post.is_ai_generated ? 'border-village-200 bg-village-50 dark:bg-village-900/30' : ''}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {post.is_ai_generated && (
              <span className="badge bg-village-100 text-village-700 dark:bg-village-900/40 dark:text-village-300">Village Elder</span>
            )}
      ...
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
        <button onClick={handleUpvote} className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-village-600 dark:text-gray-400 transition-colors">
          <ThumbsUp size={14} />
          <span>{upvotes}</span>
        </button>
        <button onClick={loadComments} className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-village-600 dark:text-gray-400 transition-colors">
      ...
      {showingComments && (
        <div className="mt-3 space-y-2">
          {comments.map((c) => (
            <div key={c.id} className="flex items-start gap-2 pl-3 border-l-2 border-amber-100 dark:border-gray-700">
              <div className="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs font-bold shrink-0">
