...
> **Status re-check — 2026-07-19** (verified against current `main`): The
> accessibility work below shipped in main via a different implementation
> (`components/TextToSpeech.tsx` + Settings toggles), and `/study` is already
> deprecated — so the "✅ Shipped (branch `claude/ux-accessibility`)" section is
> **superseded**; that branch has been retired and this doc preserved. The demo
> **blocker is RESOLVED** — migrations `004→009` are applied (see `BUGS.md`).
> Of the "🔜 Spec'd" items, **none are built yet** — including #3 *Village Fire
> momentum meter* (only marketing guide pages mention "momentum"; there is no
> `/momentum` route). All five remain open expansion targets.
